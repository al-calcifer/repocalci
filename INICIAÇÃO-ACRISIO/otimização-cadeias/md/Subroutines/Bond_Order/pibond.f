...
c
     ...
     &,XSJK(250),XSIL(250),XSJL(250),CJ(3),CK(3),CL(3)
...
C####
ccccc QQE=QI-2.0D0
ccccc QQM=QQE/(QI-XQM)
ccccc ALI=ATT*EXP(0.05D0*QQE*QQM)
ccccc DALI=ALI*0.05D0*QQM*(2.0D0-QQM)
C####
                                    ALI=1.0D0
                                    IF(QI.GT.ATT) THEN
                                         DTEMP=PQ*(QI-ATT)
...
cccc QQE=QJ-2.0D0
cccc QQM=QQE/(QJ-XQM)
cccc ALJ=ATT*EXP(0.05D0*QQE*QQM)
cccc DALJ=ALJ*0.05D0*QQM*(2.0D0-QQM)
                                    ALJ=1.0D0
                                    IF(QJ.GT.ATT) THEN
                                         DTEMP=PQ*(QJ-ATT)
...
C
                     ...
                     DO 220 K=JBEGIN,JEND
                          ...
                          NK=NK+1
cleon: put in cut-off smoothing
c                         IF(ABS(SINK(NK)).LT.1.0D-01) GO TO 220
c                         write to be equivalent for sinlo=sinhi=.1
c                         be sure to exclude the crashing case sink(nk)=0
                          zf1=1.d0
                          zf1dct=0.d0
                          if(sink(nk).lt.sinhi)then
                             zf1=0.d0
                             if(sink(nk).le.sinlo)goto 220
                             zfac=pi/(sinhi-sinlo)
                             zarg=zfac*(sink(nk)-sinhi)
                             zf1=(1.d0+cos(zarg))*.5d0
                             zf1dct=
     &                        0.5d0*sin(zarg)*zfac*cosk(nk)/sink(nk)
                          endif
cleon
...
C
                          ...
                          NL=0
                               DO 210 L=LBEGIN,LEND
                                    ...
                                    NL=NL+1
cleon: put in cut-off smoothing
c                                   IF(ABS(SINL(NL)).LT.1.0D-01)
c     &                                 GO TO 210
c                                   write to be equivalent for sinlo=sinhi=.1
c                                   be sure to exclude sinl(nl)=0
                                    zf2=1.d0
                                    zf2dct=0.d0
                                    if(sinl(nl).lt.sinhi)then
                                       zf2=0.d0
                                       if(sinl(nl).le.sinlo)goto 210
                                       zfac=pi/(sinhi-sinlo)
                                       zarg=zfac*(sinl(nl)-sinhi)
                                       zf2=(1.d0+cos(zarg))*.5d0
                                       zf2dct=0.5d0*sin(zarg)*zfac
     &                                  *cosl(nl)/sinl(nl)
                                    endif
cleon
...
C
                                    ...
                                    BT=(1.0d0-CW*CW)
cleon: add smoothing
c                                    BTOR=BTOR+BT*FCK*FCL
                                    BTOR=BTOR+BT*FCK*FCL*zf1*zf2
cleon
C   
...
C
     ...
     &                                     -CK(1)*CRLY+CL(1)*CRKY
cleon: add smoothing terms
c                                    AA=-VATT*2.0d0*CW/T1*ATOR
c     &                                  *FCL*FCK
c                                    AAA1=VATT*BT*ATOR
C                                    AT2=AA*T2
C
...
C                                    RP5=-DT1DIL*AT2
     ...
     &                               zf1*zf2dct*dctji(nl))
                                    RP2=-DT1DIK*AT2
     &                                 +AAA1*FCL*DFCK/RCK*zf1*zf2
     &                               +aaa1*fcl*fck*
     &                               zf1dct*dctik(nk)*zf2
                                    RP3=-DT1DJL*AT2
     &                                 +AAA1*FCK*DFCL/RCL*zf1*zf2
     &                               +aaa1*fcl*fck*
     &                               zf1*zf2dct*dctjl(nl)
                                    RP4=-DT1DJK*AT2
     &                               +aaa1*fcl*fck*
