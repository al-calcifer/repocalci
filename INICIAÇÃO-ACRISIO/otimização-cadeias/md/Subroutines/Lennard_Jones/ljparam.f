...
c     potential which does not conserve energy
      ijump=0
cleon

c Evaluate the LJ constants: units are in Ang. and eV
c Boltzman constant kb= 8.6173324×10−5 eV/K
c kb=(1/11605) eV/K
...
cccc Acrisio
c      Do i=1,ntypes
c        write(*,*) (sig(i,j),j=1,ntypes)
c      Enddo
c    
c      Do i=1,ntypes
c        write(*,*) (eps(i,j),j=1,ntypes)
c      Enddo
c
cccc ccccccc
...
cleon:
c                    if xmm=0, this incorrectly excluded atoms within
c                    distance RLL from being computed
                     xmms(i,j)=max(xmms(i,j),0.d0)
cleon
                     XM(i,j) = XMM(i,j)
...
C
      ...
      do i=1,ktmax
           do j=1,ktmax
cleon: substract cut-off potential to make the potential continuous
                 vcut=0.d0
                 if(ijump.eq.0 .and. rmaxlj(i,j).gt.0.d0)then
                    r6 = (sig(i,j)/rmaxlj(i,j))**3
                    vcut = eps(i,j)*r6*(r6-1.0d0)
                 endif
cleon
