...
cleon: initialize the straining subroutine
      call coord_read_hook
cleon
...
c     Also write out potential energy.
c      WRITE(9,1500) LSTEP,EETOT/FLOAT(NP),
c     &   ENPR*XX/6.0d0/DELTSQ*ECONV,DELTA,TIME
      WRITE(9,1500) LSTEP,EETOT/FLOAT(NP),TOTE/FLOAT(NP),
...
c     Too much writing to the screen already
c      IF((KFLAG.NE.6).AND.(KFLAG.NE.8)) THEN
c           WRITE(6,*) 'TOTE: ',TOTE
c      ENDIF
c     Write some full info instead on occasion
      if(mod(lstep,max(1,(kvc+15)/30)).eq.0)
     & write(6,1500)LSTEP,EETOT/FLOAT(NP),TOTE/FLOAT(NP),
     & ENPR*XX/6.0d0/DELTSQ*ECONV
...
c     Add the potential energy field
c 1500 FORMAT(I6,F14.8,F10.3,2F9.3)
 1500 FORMAT(I6,2F14.8,F10.3,2F9.3)
