...
C
      ...
      enddo
C
c      WRITE(801,*) LCHK,(KTYPE(I),I=1,NP)
...
405                  CONTINUE
                     K=K+1
cleon: moved up from below and changed output:
                     if(k.gt.nlmax) then
...
C
           ...
           KEND=K
cleon: removed
c           if(kend.gt.nlmax) then
c                 write(*,*) 'kend exceeds nlmax'
c                 write(*,*) 'kend,nlmax = ',kend,nlmax
c                 write(*,*) 'increase nlmax and recompile' 
c                 include 'close.inc' 
c                 stop 
c           endif
cleon
...
322        CONTINUE
321   CONTINUE
cccccc Linha acrisio cccc
c      Do k=1,kend
c       Write(199,*) RNP(k,1), RNP(k,2), RNP(k,3)
c      Enddo
c       aba=1111111
c       Write(199,*) aba
c      Do k=1,kend
c       Write(199,*) RPP(k,1), RPP(k,2), RPP(k,3)
c      Enddo
cccccc Linha acrisio ccccc
C
C
      if(noa(1)+noa(2).ne.0) call pibond
      if(noa(3)+noa(4).ne.0) call sili_germ
C
      RETURN
C
      END
